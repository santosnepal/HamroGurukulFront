...
const StudentApplyLeave = ({logout,isAuthenticated  })=>{
  ...
  const bhupu = await axios.get(`http://127.0.0.1:8000/api/getstudentleave/${loadUserid()}`,config);
  updateHistory(bhupu);
  }
  ...
  const body = JSON.stringify({leave_date,leave_message,student_id});
  console.log(body);
  const res = await axios.post("http://127.0.0.1:8000/api/addstudentleave",body,config);
  if(res.status===200){
